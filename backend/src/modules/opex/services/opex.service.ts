import { Injectable, NotFoundException, BadRequestException, Logger, HttpException, Inject, forwardRef } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository, DataSource, EntityManager, MoreThan, In } from 'typeorm';
import { Cron, CronExpression } from '@nestjs/schedule';
import { YearlyOpex, MonthlyOpex, OpexItem, OpexType, OpexRelationshipType } from '../../../entities';
import { CreateYearlyOpexDto, UpdateYearlyOpexDto, UpdateMonthDataDto } from '../dto';
...
export class OpexService {
  ...
  async create(createYearlyOpexDto: CreateYearlyOpexDto): Promise<YearlyOpex> {
    try {
      ...
      if (existing) {
        throw new OpexConflictException(`OPEX data for year ${createYearlyOpexDto.year} already exists`);
      }
      ...
      yearlyOpex.months = createYearlyOpexDto.months.map(monthDto => {
        if (monthDto.month < 1 || monthDto.month > 12) {
          throw new OpexValidationException(`Invalid month: ${monthDto.month}. Month must be between 1 and 12`);
        }
  ...
  async findByYear(year: number): Promise<YearlyOpex> {
    ...
    if (!yearlyOpex) {
      ...
      console.log(`[DEBUG] ${year}년 월별 데이터 부족 (현재: ${yearlyOpex.months?.length || 0}개월), 12개월로 보완`);
      yearlyOpex = await this.ensureAllMonthsExist(yearlyOpex);
    }

    console.log(`[DEBUG] ${year}년 데이터 로드: ${yearlyOpex.months?.length || 0}개월, 총 OPEX 항목: ${yearlyOpex.months?.reduce((sum, m) => sum + (m.opexItems?.length || 0), 0) || 0}개`);
    ...
    if (yearlyOpex.months) {
      for (const month of yearlyOpex.months) {
        try {
          ...
          const actualEmployeeCount = await this.employeeService.getActiveEmployeeCountByMonth(year, month.month);
          const oldCount = month.employeeCount;
          
          // DB에 저장된 값을 실제 계산된 값으로 업데이트
          month.employeeCount = actualEmployeeCount;
          
          console.log(`[DEBUG] ${month.month}월 직원수 업데이트: ${oldCount}명 → ${actualEmployeeCount}명 (동적계산)`);
        } catch (error) {
    ...
    yearlyOpex.months?.forEach(month => {
      if (month.opexItems && month.opexItems.length > 0) {
        const indirectTotal = month.opexItems.filter(item => item.type === OpexType.INDIRECT).reduce((sum, item) => sum + item.amount, 0);
        const directTotal = month.opexItems.filter(item => item.type === OpexType.DIRECT).reduce((sum, item) => sum + item.amount, 0);
        console.log(`[DEBUG] ${month.month}월: 직원수 ${month.employeeCount}명, Indirect ${indirectTotal}원, Direct ${directTotal}원, 확정여부: ${month.confirmed}`);
      } else {
        console.log(`[DEBUG] ${month.month}월: 직원수 ${month.employeeCount}명, 데이터 없음, 확정여부: ${month.confirmed}`);
      }
    });
  ...
  private validateOpexItem(item: OpexItem): void {
    ...
    if (relationships.length !== 1) {
      throw new Error(`OpexItem(ID: ${item.id || 'new'})은 정확히 하나의 관계만 가져야 합니다. 현재: ${relationships.length}개`);
    }
    ...
    switch (item.relationshipType) {
      case OpexRelationshipType.MONTHLY_OPEX:
        if (!item.monthlyOpex) {
          throw new Error(`OpexItem(ID: ${item.id || 'new'})의 relationshipType이 MONTHLY_OPEX인데 monthlyOpex가 없습니다.`);
        }
        break;
      case OpexRelationshipType.PPE_INDIRECT:
        if (!item.projectPPEIndirect) {
          throw new Error(`OpexItem(ID: ${item.id || 'new'})의 relationshipType이 PPE_INDIRECT인데 projectPPEIndirect가 없습니다.`);
        }
        break;
      case OpexRelationshipType.PPE_DIRECT:
        if (!item.projectPPEDirect) {
          throw new Error(`OpexItem(ID: ${item.id || 'new'})의 relationshipType이 PPE_DIRECT인데 projectPPEDirect가 없습니다.`);
        }
        break;
    }
  }

  private async ensureAllMonthsExist(yearlyOpex: YearlyOpex): Promise<YearlyOpex> {
    ...
    for (let month = 1; month <= 12; month++) {
      if (!existingMonths.includes(month)) {
        ...
        if (!existingMonth) {
          ...
          console.log(`[DEBUG] ${month}월 MonthlyOpex 새로 생성: ID ${savedMonth.id}`);
        } else {
          yearlyOpex.months.push(existingMonth);
          console.log(`[DEBUG] ${month}월 MonthlyOpex 기존 데이터 사용: ID ${existingMonth.id}`);
        }
      }
    }
  ...
  async updateMonthData(year: number, month: number, data: any): Promise<MonthlyOpex> {
    ...
    try {
      ...
      const incomingItems = [
        ...(data.indirect || []).map(item => ({ ...item, type: OpexType.INDIRECT })),
        ...(data.direct || []).map(item => ({ ...item, type: OpexType.DIRECT }))
      ].filter(item => item.category && item.category.trim() && item.amount > 0);
      ...
      if (existingItems.length > 0) {
        const deleteResult = await this.opexItemRepository.delete(existingItems.map(item => item.id));
        console.log(`[DEBUG] 기존 항목 삭제 완료: ${deleteResult.affected || 0}개`);
      } else {
      ...
      for (const incomingItem of incomingItems) {
        ...
        console.log(`[DEBUG] 저장된 항목: ${savedItem.id} - ${savedItem.category}`);
      }
      ...
      finalItems.forEach(item => {
        console.log(`[DEBUG] - ID: ${item.id}, Category: ${item.category}, Amount: ${item.amount}`);
      });
   ...
   */
  async updateMonthDataV2(year: number, month: number, dto: UpdateMonthDataDto): Promise<MonthlyOpex> {
    ...
    return await this.dataSource.transaction(async (manager: EntityManager) => {
      try {
        ...
        if (!monthlyOpex) {
          ...
          const yearlyOpex = await this.findByYear(year);
          monthlyOpex = yearlyOpex.months.find(m => m.month === month) || null;
        ...
        try {
          const actualEmployeeCount = await this.employeeService.getActiveEmployeeCountByMonth(year, month);
          
          if (monthlyOpex.employeeCount !== actualEmployeeCount) {
            console.log(`[DEBUG] 직원수 변경: ${monthlyOpex.employeeCount}명 → ${actualEmployeeCount}명`);
        ...
        // 5. 추가/수정 처리
        const allItems = [
          ...(dto.indirect || []).map(item => ({ ...item, type: OpexType.INDIRECT })),
          ...(dto.direct || []).map(item => ({ ...item, type: OpexType.DIRECT }))
        ];

        console.log(`[DEBUG] 처리할 항목 수: ${allItems.length}개`);

        // 6. 기존 항목별 최대 번호 확인 (새 항목 추가 시 사용)
        const existingIndirectItems = existingItems.filter(item => item.type === OpexType.INDIRECT);
        const existingDirectItems = existingItems.filter(item => item.type === OpexType.DIRECT);
        ...
        for (const itemData of allItems) {
          ...
          if (!itemData.category?.trim() || itemData.amount <= 0) {
            console.log(`[DEBUG] 빈 데이터 제외: ${itemData.category || 'empty'}`);
            continue;
          }

          if (itemData.id && itemData.id > 0 && existingItemsMap.has(itemData.id)) {
            // 기존 항목 업데이트 (변경 감지)
            ...
            if (hasChanges) {
              ...
              console.log(`[DEBUG] 업데이트: ID ${itemData.id} - ${itemData.category} (변경사항 있음)`);
            } else {
              console.log(`[DEBUG] 변경 없음: ID ${itemData.id} - ${itemData.category} (저장 건너뜀)`);
            }
          } else {
            ...
            console.log(`[DEBUG] 추가: ${itemData.category} (${itemData.type})`);
          }
        }
        ...
        if (totalChanges === 0) {
          ...
          console.log(`[DEBUG] 총 변경사항: 직원수 ${employeeCountChanged ? '변경' : '유지'}, 항목 ${itemsToSave.length}개 저장, ${dto.deleteIds?.length || 0}개 삭제`);
        }
        ...
        console.log(`[DEBUG] updateMonthDataV2 완료: 총 ${updatedMonthlyOpex.opexItems?.length || 0}개 항목`);
        return updatedMonthlyOpex;
      } catch (error) {
        this.logger.error(`updateMonthDataV2 실패: ${error.message}`, error.stack);
        if (error instanceof HttpException) {
          throw error;
        }
  ...
  async confirmMonth(year: number, month: number): Promise<any> {
    ...
    return await this.dataSource.transaction(async (manager: EntityManager) => {
      try {
        ...
        this.logger.log(`확정 전 ${month}월 OPEX 항목 수: ${monthlyOpex.opexItems?.length || 0}`);
        
        // OPEX 항목이 없으면 확정 불가
        if (!monthlyOpex.opexItems || monthlyOpex.opexItems.length === 0) {
          throw new OpexConfirmationException(year, month, 'No OPEX items to confirm');
        }
      
      if (monthlyOpex.confirmed) {
        console.log(`[DEBUG] ${month}월은 이미 확정된 상태입니다. 재확정을 진행합니다.`);
      }
      ...
      console.log(`[DEBUG] 확정된 월 ${month}의 OPEX 항목 수: ${monthlyOpex.opexItems?.length || 0}`);
      console.log(`[DEBUG] 확정된 월 OPEX 항목 상세:`, monthlyOpex.opexItems?.map(item => ({
        id: item.id,
      ...
      await this.projectConfirmedDataToFutureMonthsInTransaction(manager, monthlyOpex, month);
  ...
  private async projectConfirmedDataToFutureMonthsInTransaction(manager: EntityManager, confirmedMonthlyOpex: MonthlyOpex, confirmedMonth: number): Promise<void> {
    console.log(`[DEBUG] 확정 월 ${confirmedMonth}의 데이터를 이후 달에 반영 시작 (트랜잭션, Batch 최적화)`);
    
    console.log(`[DEBUG] ${confirmedMonth}월 OPEX 항목 수: ${confirmedMonthlyOpex.opexItems?.length || 0}`);
    console.log(`[DEBUG] ${confirmedMonth}월 OPEX 항목 상세:`, confirmedMonthlyOpex.opexItems?.map(item => ({ 
      category: item.category, 
      amount: item.amount, 
      type: item.type 
    })));
    ...
    console.log(`[DEBUG] 미래 월들: ${futureMonths.map(m => m.month).join(', ')}`);
    ...
    console.log(`[DEBUG] Batch 삭제 시작: ${futureMonthIds.length}개 월의 기존 항목`);
    
    const batchDeleteResult = await manager.delete(OpexItem, { 
      monthlyOpex: { id: In(futureMonthIds) } 
    });
    console.log(`[DEBUG] Batch 삭제 완료: ${batchDeleteResult.affected || 0}개 항목 삭제`);
    ...
    console.log(`[DEBUG] Batch 저장 시작: 총 ${allCopiedItems.length}개 항목 (${futureMonths.length}개 월)`);
    ...
    console.log(`[DEBUG] 월별 항목 수:`, futureMonths.map(fm => 
      `${fm.month}월: ${allCopiedItems.filter(item => item.monthlyOpex?.id === fm.id).length}개`
    ).join(', '));
    
    console.log(`[DEBUG] 확정 월 ${confirmedMonth} 데이터 반영 완료 (트랜잭션, Batch 최적화)`);
  }

  private async projectConfirmedDataToFutureMonths(confirmedMonthlyOpex: MonthlyOpex, confirmedMonth: number): Promise<void> {
    console.log(`[DEBUG] 확정 월 ${confirmedMonth}의 데이터를 이후 달에 반영 시작`);
    ...
    for (const futureMonth of futureMonths) {
      ...
      const deleteQuery = this.opexItemRepository
        ...
        .delete()
        .from(OpexItem)
        .where('monthlyOpexId = :monthlyOpexId', { monthlyOpexId: futureMonth.id });
      
      const deleteResult = await deleteQuery.execute();
      console.log(`[DEBUG] ${futureMonth.month}월 기존 항목 ${deleteResult.affected || 0}개 삭제 완료`);
      ...
      console.log(`[DEBUG] ${futureMonth.month}월에 복사할 항목 수: ${copiedItems.length}`);
      console.log(`[DEBUG] ${futureMonth.month}월 MonthlyOpex ID: ${futureMonth.id}`);
      
      const savedItems = await this.opexItemRepository.save(copiedItems);
      
      console.log(`[DEBUG] ${futureMonth.month}월에 ${savedItems.length}개 항목 저장 완료`);
      console.log(`[DEBUG] ${futureMonth.month}월 저장된 항목 ID들:`, savedItems.map(item => item.id));
      
      // 저장 직후 실제 DB에서 확인
      const verifyCount = await this.opexItemRepository.count({ 
        where: { monthlyOpex: { id: futureMonth.id } } 
      });
      console.log(`[DEBUG] ${futureMonth.month}월 DB 실제 저장 확인: ${verifyCount}개 항목`);
    }
  ...
  async getOpexItemsForDebug(year: number) {
    ...
    const result = {
      ...
      totalItems: yearlyOpex.months.reduce((sum, m) => sum + (m.opexItems?.length || 0), 0),
      monthlyDetails: yearlyOpex.months.map(month => ({
        ...
        items: month.opexItems?.map(item => ({
          id: item.id,
          category: item.category,
          amount: item.amount,
          type: item.type,
    ...
    console.log(`[DEBUG] 디버깅 API 호출: ${year}년도 총 ${result.totalItems}개 OPEX 항목`);
  ...
  async resetConfirmedStatus(year: number, month: number) {
    ...
    console.log(`[DEBUG] ${year}년 ${month}월 confirmed 상태: ${previousStatus} → false`);
  ...
  async getRawItemCount(year: number) {
    ...
    try {
      ...
      const totalItems = result.reduce((sum, row) => sum + parseInt(row.item_count), 0);
  ...
  async createTestMonthData(year: number, month: number) {
    ...
    // 기존 OPEX 항목들 삭제
    if (monthlyOpex.opexItems && monthlyOpex.opexItems.length > 0) {
      await this.opexItemRepository.delete({ monthlyOpex: { id: monthlyOpex.id } });
    ...
    console.log(`[DEBUG] ${month}월에 ${savedItems.length}개 테스트 항목 저장 완료`);
    ...
    if (!updatedMonthlyOpex) {
      throw new Error(`테스트 데이터 생성 후 ${year}년 ${month}월 데이터를 찾을 수 없습니다.`);
    }
    ...
    console.log(`[DEBUG] - Indirect: ${indirectTotal}원 (${updatedMonthlyOpex.opexItems.filter(item => item.type === OpexType.INDIRECT).length}개 항목)`);
    console.log(`[DEBUG] - Direct: ${directTotal}원 (${updatedMonthlyOpex.opexItems.filter(item => item.type === OpexType.DIRECT).length}개 항목)`);
    console.log(`[DEBUG] - 총 ${updatedMonthlyOpex.opexItems.length}개 항목, 직원수: ${updatedMonthlyOpex.employeeCount}명`);
  ...
  async createNewYearData() {
    ...
    console.log(`[SCHEDULER] ${currentYear}년 1월 1일 - ${currentYear}년 OPEX 데이터 자동 생성 시작`);
    ...
    try {
      ...
      if (!existingYearlyOpex) {
        ...
        console.log(`[SCHEDULER] ${currentYear}년 OPEX 데이터 자동 생성 완료 - ID: ${newYearlyOpex.id}`);
      } else {
        console.log(`[SCHEDULER] ${currentYear}년 OPEX 데이터가 이미 존재합니다.`);
      }
    } catch (error) {
      console.error(`[SCHEDULER] ${currentYear}년 OPEX 데이터 생성 실패:`, error);
    }
  }
  ...
  async testDirectDelete(year: number, month: number): Promise<any> {
    ...
    console.log(`[DEBUG] 삭제할 항목들:`, itemsToDelete.map(item => ({ id: item.id, category: item.category })));
    ...
    const deleteResult = await this.opexItemRepository.delete(itemsToDelete.map(item => item.id));
  ...
  async clearAllOpexData(year: number): Promise<any> {
    ...
    console.log(`[DEBUG] ${year}년 YearlyOpex 삭제: ${deleteResult.affected}개`);
  ...
  async deleteOpexItem(id: number): Promise<{ success: boolean; message: string }> {
    const item = await this.getOpexItem(id);
