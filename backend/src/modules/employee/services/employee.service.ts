...
export class EmployeeService {
  ...
  private async generateEmpNo(): Promise<string> {
    ...
    // 3자리로 패딩 (최대 999명/년)
    if (nextNumber > 999) {
      throw new Error(`연도별 사번 한계(999)를 초과했습니다. 현재 연도: ${year}`);
    }
   ...
   */
  async getActiveEmployeeCountByMonth(year: number, month: number): Promise<number> {
    // 해당 월의 시작일과 종료일 계산
    ...
    console.log(`[DEBUG] 직원수 계산: ${year}년 ${month}월 (${startOfMonth.toISOString().split('T')[0]} ~ ${endOfMonth.toISOString().split('T')[0]})`);
    ...
    for (const employee of allEmployees) {
      ...
      if (joinDate > endOfMonth) {
        console.log(`[DEBUG] 입사일 이후: ${employee.name} (입사일: ${joinDate.toISOString().split('T')[0]})`);
        continue;
      }
      ...
      if (employee.endDate) {
        ...
        if (endDate < startOfMonth) {
          console.log(`[DEBUG] 퇴사일 이전: ${employee.name} (퇴사일: ${endDate.toISOString().split('T')[0]})`);
          continue;
        }
      }
      ...
      const isOnLeaveForEntireMonth = employee.leaveRequests?.some(request => {
      ...
      activeCount++;
      console.log(`[DEBUG] 재직 중: ${employee.name} (상태: ${employee.status})`);
    }
